...
function insertionSort(nums =[]) {
    ...
    for (let i = 1; i < nums.length; i++) {
        // this is the start of the 1st for loop which will loop through nums and set the temp to the current index.
        let temp = nums[i]
        let j = i -1;
        ...
        //  if j is bigger than i then we are setting it to the right
        nums[j + 1] = temp
    }
    return nums
