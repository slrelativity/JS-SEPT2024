...
function musicRuntime(busDuration, songDurations) {
    const targetTime = busDuration - 30; // Time to look for pairs
    const songMap = new Map(); // To store song duration and index
    let bestPair = [-1, -1]; // Default pair if no valid pair is found
    let longestSong = -1; // To track the longest song in a valid pair

    for (let i = 0; i < songDurations.length; i++) {
        const currentSong = songDurations[i];
        const complement = targetTime - currentSong; // 200

        // Check if the complement exists in the map
        if (songMap.has(complement)) {
            const complementIndex = songMap.get(complement);

            // Check if the current pair has a song longer than the best found so far
            const maxSongInPair = Math.max(currentSong, complement);
            if (maxSongInPair > longestSong) {
                bestPair = [complementIndex, i];
                longestSong = maxSongInPair;
            }
        }

        // Add current song duration and index to the map
        songMap.set(currentSong, i);
    }
